...
On first activation the extension downloads the server for your platform and
keeps it updated.

### Standalone binary
Prebuilt for Linux x64, macOS arm64/x64 and Windows x64:
...
Download from the [nightly release](https://github.com/xoxorwr/dmd-lsp/releases/tag/nightly)
(with `SHA256SUMS`), or build from source below.

## Usage

### VS Code
The extension starts the server for `.d`/`.di` automatically. Settings:
...
### Other editors
Point any LSP client at `dmd-lsp` (stdio). Supported:
